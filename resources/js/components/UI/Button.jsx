...
const Button = ({ children, onClick, className = '', variant = 'primary', disabled = false, loading = false, type = 'button', ...props }) => {
    const variants = {
    ...
    return (
        <button
            ...
            className={`${variants[variant] || variants.primary} ${className} flex items-center justify-center gap-2`}
            {...props}
        >
            {loading && (
