...
const ExtraIncomeReceipt = () => {
    ...
    useEffect(() => {
        const fetchReceiptData = async () => {
            try {
                const response = await axios.get(`/api/extra-incomes/${id}`);
    ...
    const handleDownload = () => {
        ...
        if (data?.form_number) {
            document.title = `Income_Receipt_${data.form_number}`;
        } else {
            document.title = `Income_Receipt_${id}`;
        }
        window.print();
        ...
        <div className="bg-white min-h-screen">
            <div className="print:hidden p-4 bg-slate-800 text-white flex justify-between items-center sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-bold">Income Receipt</h1>
                </div>
...
export default ExtraIncomeReceipt;
