        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100">
                <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                    <h3 className="text-xl font-bold text-slate-800">Lifecycle Action</h3>
                    ...
                    {formData.status === 'Transferred' && (
                        <div className="space-y-4 pt-1 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="flex gap-2 p-1 bg-slate-100 rounded-lg">
                                <button type="button" onClick={() => setIsInternalTransfer(true)} className={`flex-1 py-1.5 text-[11px] font-bold rounded-md transition-all ${isInternalTransfer ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>INTERNAL</button>
                                <button type="button" onClick={() => setIsInternalTransfer(false)} className={`flex-1 py-1.5 text-[11px] font-bold rounded-md transition-all ${!isInternalTransfer ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>EXTERNAL</button>
                            </div>
                            {isInternalTransfer ? (
                                <div>
